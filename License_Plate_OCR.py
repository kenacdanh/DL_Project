...
# Khởi tạo Flask, Socket.IO, và Flask-RESTful Api
app = Flask(__name__)
...
def reset_db():
    conn = sqlite3.connect('vehicle_log.db')
    cursor = conn.cursor()
    cursor.execute("DROP TABLE IF EXISTS VehicleLogs")
    init_db()  # Gọi lại hàm init_db để tạo lại bảng
    conn.close()
...
def initialize_model():
    global model, device
    model_path = r'D:\DLproject\yolov5\runs\train\exp\weights\best.pt'
...
def process_video_stream():
    global model, device
    video_source = r"D:\DLproject\videos\cong_vao.mp4"
...
def process_video_stream_2():
    global model, device
    video_source = r"D:\DLproject\videos\cong_ra.mp4"
    cap = cv2.VideoCapture(video_source)
...
# ==========================
# Thêm các API Endpoint ở đây
# ==========================

# Resource cho danh sách xe
class VehicleListResource(Resource):
    def get(self):
        try:
            conn = sqlite3.connect('vehicle_log.db')
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM VehicleLogs ORDER BY entry_time DESC")
            ...
            logging.error(f"Lỗi khi lấy danh sách xe: {e}")
            return {'message': str(e)}, 500

# Resource cho log xe vào
class VehicleEntryResource(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('plate', required=True, help="Biển số xe không được để trống!")
        args = parser.parse_args()
        plate = args['plate']
        log_entry(plate)
        return {'message': 'Ghi nhận xe vào thành công'}, 201

# Resource cho log xe ra
class VehicleExitResource(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('plate', required=True, help="Biển số xe không được để trống!")
        args = parser.parse_args()
        plate = args['plate']
        log_exit(plate)
        return {'message': 'Ghi nhận xe ra thành công'}, 201

# Thêm các resource vào API
api.add_resource(VehicleListResource, '/api/vehicles')
api.add_resource(VehicleEntryResource, '/api/vehicles/entry')
api.add_resource(VehicleExitResource, '/api/vehicles/exit')

# ==========================
# Kết Thúc Thêm Các API Endpoint
# ==========================

# Hàm main
if __name__ == "__main__":
    reset_db()  # Chạy hàm này một lần để tạo lại bảng
    try:
    ...
    # Khởi tạo thread cho video stream thứ nhất
    video_thread_1 = threading.Thread(target=process_video_stream)
    video_thread_1.daemon = True
    video_thread_1.start()

    # Khởi tạo thread cho video stream thứ hai
    video_thread_2 = threading.Thread(target=process_video_stream_2)
    video_thread_2.daemon = True
    video_thread_2.start()

    # Khởi chạy Socket.IO với Flask-RESTful
